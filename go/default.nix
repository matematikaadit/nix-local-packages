{ stdenv, lib, fetchurl, bison, glibc, bash, coreutils, makeWrapper, tzdata, iana_etc, perl, Security }:
  ...
  installPhase = ''
    ...
    for os in windows darwin linux; do
        for arch in 386 amd64; do
            CGO_ENABLED=0 GOOS=$os GOARCH=$arch go tool dist install -v runtime
            CGO_ENABLED=0 GOOS=$os GOARCH=$arch go install -v -a std
        done
    done
  '';
